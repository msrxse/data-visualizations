...
export function DateHistogram({
  ...
  const binnedData = useMemo(() => {
    ...
    return bin<Data, Date>() // construct iterator - adding the data creates the bins / .value() will add the aggregation function
      .value(xValue)
      ...
      .map((array) => ({
        ...
        y: sum(array, yValue), // yValue wont change between rerenders - is defined outside
  ...
  useEffect(() => {
    ...
    brush.on('brush end', (event) => {
      // On end the event.selection would be null
      setBrushExtent(event.selection && event.selection.map(xScale.invert)) // since event.selection is a selection - you can map()
    })
